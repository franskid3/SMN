...
live_tab, historical_tab = st.tabs(
    ["⚡ REAL-TIME MONITORING CONTROL", "📊 ASSET DATA TRACE DESK"]
)
    ...
    def render_live_pipeline():
        ...
        if not snapshot_df.empty:
            for _, row in snapshot_df.iterrows():
                ...
                if top == "SMN/EDU":
                    edu_payload = pay
                elif top == "SMN/HEARTBEAT":
                    heartbeat_payload = pay
                elif "SLOT" in top:
                    active_slots[top] = pay
        ...
        # Convert Hardware Wh register directly to kWh for uniform display units
        hardware_energy_kwh = float(edu_payload.get("energy", 0.0)) / 1000.0

        # Interlock and Generator Status Evaluation
        is_gen_active = (
            edu_payload.get("gen1_mask", 0) == 1
            or edu_payload.get("gen2_mask", 0) == 1
        )
        system_status = heartbeat_payload.get("system", "OFFLINE")
        ...
        with m2:
            ...
            <div class='metric-card'>
                ...
                <div class='metric-value'>{hardware_energy_kwh:.3f} kWh</div>
            </div>""",
                unsafe_allow_html=True,
            )
        with m3:
        ...
        # Redundant Audit Desk
        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("📟 Energy Meter Validation Audit"):
            (a1,) = st.columns(1)
            with a1:
                st.metric(
                    "Hardware Energy Register (📟 HW)",
                    f"{edu_payload.get('energy', 0)} Wh",
                )

        # Interactive Sub-Station Slot Grid Matrix
        st.markdown(
            "<br><h3>🔋 Distributed Charging Racks (DCU Asset Grid)</h3>",
            unsafe_allow_html=True,
        )

        selected_rack = st.selectbox(
            "Isolate Distribution Control Unit Rack:",
            ["DCU Rack Alpha (Nodes 1-12)"],
        )
        ...
        for idx in range(1, 13):
            ...
            with col_target:
                if slot_topic in active_slots:
                    ...
                    card_style = (
                        "slot-charging" if "CHARGING" in status else "slot-standby"
                    )
                    if "FAULT" in status:
                        card_style = "slot-fault"
...
with historical_tab:
    ...
    with c_left:
        selected_date = st.date_input(
            "Select Historical Query Window:", value=datetime.date.today()
        )

        # Pull distinct BMS IDs logged on this day
        available_bms_ids = get_unique_bms_ids(selected_date)
        selected_bms_id = st.selectbox(
            "Select Target Battery Asset Tracking String (BMS_ID):",
            (
                available_bms_ids
                if available_bms_ids
                else ["No battery assets logged today"]
            ),
        )
    ...
    if not hist_station_df.empty:
        ...
        # Display the text values written out clearly
        (h_m1,) = st.columns(1)
        with h_m1:
            st.markdown(
                f"""
            <div class='metric-card' style='border-left: 6px solid #E53E3E;'>
                <div class='metric-title'>📟 Hardware Reported Total Energy</div>
                <div class='metric-value'>{final_hw_energy:.2f} kWh</div>
                <div style='color: #A0AEC0; font-size: 0.85rem; margin-top: 5px;'>Physical Register Flash-Recovery Value</div>
            </div>""",
                unsafe_allow_html=True,
        ...
        # --- GENERATOR THREE-PHASE LOAD CHART ---
        st.markdown(
            "<br>#### ⚡ Generator Phase Load Analysis (Phase 1, 2, 3)",
            unsafe_allow_html=True,
        )
        ...
        fig_phases.add_trace(
            go.Scatter(
                x=hist_station_df["timestamp"],
                y=hist_station_df["p_total_kw"],
                name="Total Load Vector (kW)",
    ...
    st.markdown("---")
    st.markdown(
        f"#### 🔋 Deep Battery Lifecycle Audit File [Asset: {selected_bms_id}]"
    )
    if selected_bms_id and selected_bms_id != "No battery assets logged today":
        trace_df = query_battery_trace(selected_date, selected_bms_id)
        ...
        if not trace_df.empty:
            fig_trace = make_subplots(
                ...
                subplot_titles=(
                    "State of Charge Lifecycle (%)",
                    "Voltage Signature (V)",
                    "Current Draw Profile (A)",
                ),
            )
            ...
            st.info(
                "Select a logged battery string to draw structural timeline traces."
            )